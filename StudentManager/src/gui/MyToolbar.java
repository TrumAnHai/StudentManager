...
public class MyToolbar extends JPanel implements ActionListener {
	...
	public MyToolbar(MyFrame _myFrame) {
		...
		this.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED, null, null), "", TitledBorder.LEADING,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals(btnCancel)) {
			...
			myFrame.getListStudent().getsList().clear();
		...
		if (command.equals(btnInsert.getText())) {
			...
			// int id =
			// this.myFrame.getListStudent().getsList().get(index1).getIdStudent()+1;
			int id = this.myFrame.getListStudent().getStudent(index1).getIdStudent() + 1;
			// endregion
			...
			if (name.equals("") || birth == null || math < 0 || math > 10 || phy < 0 || phy > 10 || che < 0 || che > 10
					|| (infoPanel.getCkbFemale().isSelected() == true && infoPanel.getCkbMale().isSelected() == true)) {
				JOptionPane.showMessageDialog(myFrame, "Invalid data");
			} else {
				int result = JOptionPane.showConfirmDialog(myFrame, "Do u want to add this info to the list?",
		...
		if (command.equals(btnEdit.getText())) {
			...
			if (index >= 0) {
				...
				double phy = Double.parseDouble(infoPanel.getTxfPhyScore().getText().trim());
				double che = Double.parseDouble(infoPanel.getTxfCheScore().getText().trim());
				if (name.equals("") || birth == null || math < 0 || math > 10 || phy < 0 || phy > 10 || che < 0
						|| che > 10 || (infoPanel.getCkbFemale().isSelected() == true
								&& infoPanel.getCkbMale().isSelected() == true)) {
					JOptionPane.showMessageDialog(myFrame, "Invalid data");
				} else {
					int result = JOptionPane.showConfirmDialog(myFrame, "Do u want to add this info to the list?",
							"Confirm", JOptionPane.YES_NO_OPTION);
		...
		if (command.equals(btnDelete.getText())) {
			int index = myFrame.getTable().getSelectedRow();
			if (index >= 0) {
				int result = JOptionPane.showConfirmDialog(myFrame, "Do u want to delete this info?", "Confirm",
